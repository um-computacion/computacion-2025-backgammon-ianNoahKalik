...
def mover_pieza(self, jugador: int, origen: int, destino: int):
        """
        Mueve una ficha del jugador de un punto origen a un destino.
        Captura ficha enemiga si aterriza sobre una sola.
        Lanza TableroError en movimientos inválidos.
        """
        # validar jugador
        ...
        # validar índices
        if not (0 <= origen < self.PUNTOS_CONTADOS):
            raise TableroError(f"Origen fuera de rango: {origen}")
        if not (0 <= destino < self.PUNTOS_CONTADOS):
